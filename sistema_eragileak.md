...
Repositorio honek **Debian 12** zerbitzari baten konfigurazio-scriptak eta dokumentazio osoa barne hartzen ditu. Bertan azaltzen dira baliabideen kudeaketa, fitxategiak partekatzea, kontenedore bidezko monitorizazio aurreratua, segurtasun automatizatua eta sare bidezko inplementazioa (PXE).
...
3. [Monitorizazioa (Cockpit, htop, Netdata)](#3-monitorizazioa-cockpit-htop-netdata)
4. [Docker: Baliabideen Mugak eta Alertak](#4-docker-baliabideen-mugak-eta-alertak)
5. [Segurtasuna eta Automatizazioa (Cron + RKHunter)](#5-segurtasuna-eta-automatizazioa-cron--rkhunter)
6. [Sare bidezko Instalazioa (PXE Zerbitzaria)](#6-sare-bidezko-instalazioa-pxe-zerbitzaria)

---

## 1. Urruneko Sarbide Grafikoa (XRDP)

Zerbitzariaren administrazio bisuala ahalbidetzen du Windows (RDP) eta Linux (Remmina) bezeroetatik. **XFCE** erabiltzen da, zerbitzarietarako ingurune grafiko arin ezin hobea delako.

```bash
# 1. XFCE ingurunearen eta XRDP zerbitzariaren instalazioa
sudo apt update
sudo apt install xfce4 xfce4-goodies xrdp dbus-x11 -y

# 2. Urruneko saioetan XFCE erabiltzera behartu
echo "xfce4-session" > ~/.xsession

# 3. Aldaketak aplikatu eta zerbitzua abiaraztean gaitu
...
## 2. Samba eta Disko-kuotak

Sare lokalean partekatutako karpeta baten konfigurazioa, biltegiratze-politika zorrotzekin erabiltzaileko (Kuotak), erro-diskoa bete ez dadin.

### 2.1. Sambaren Inplementazioa
```bash
...
### 2.2. Kuoten Inplementazioa (Quotas)
...
## 3. Monitorizazioa (Cockpit, htop, Netdata)
...
## 4. Docker: Baliabideen Mugak eta Alertak
...
Ostalariaren bolumenak estekatzen dira hardwarearen irakurketa sakona ahalbidetzeko:
```bash
...
Eskaneatzeen eta eguneratzeen programazioa gauez:
```bash
...
## 6. Sare bidezko Instalazioa (PXE Zerbitzaria)

Sistema Eragileak sare lokalaren bidez (PXE Boot) bezero berrietan automatikoki hedatzeko azpiegitura, euskarri fisikoen beharra ezabatuz.

```bash
# 1. Oinarrizko zerbitzuen instalazioa (DHCP/TFTP/Bootloader)
sudo apt install dnsmasq pxelinux syslinux-efi -y
sudo mkdir -p /srv/tftp/pxelinux.cfg

# 2. Abio-fitxategiak prestatzea
sudo cp /usr/lib/PXELINUX/pxelinux.0 /srv/tftp/
sudo cp /usr/lib/syslinux/modules/bios/ldlinux.c32 /srv/tftp/

# 3. Dnsmasq-en konfigurazioa
